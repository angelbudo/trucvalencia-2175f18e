...
import { playerTotalEnvit } from "./_game/deck.ts";
import {
...
// Regla estricta: 8000 ms exactes d'espera per al segon de la parella a la
// 1a baza quan NO té envit propi. Si té envit (≥30) i pot cantar-lo,
// `shouldApplyFirstTrickWait` retorna false i actua immediatament.
const SECOND_PLAYER_WAIT_MIN_MS = 8000;
const SECOND_PLAYER_WAIT_MAX_MS = 8000;
...
function shouldApplyFirstTrickWait(state: MatchState, actor: PlayerId): boolean {
  ...
  // Excepción (iniciativa directa): si el bot puede cantar envit AHORA y
  // tiene envit alto (≥30), NO espera — actúa inmediatamente. Sólo se
  // aplica la espera de 8s cuando el bot NO tiene envit propio.
  const canEnvitNow = legalActions(state, actor).some(
    (a: any) => a.type === "shout" && a.what === "envit",
  );
  if (canEnvitNow && playerTotalEnvit(r, actor) >= 30) return false;
