...
function TableSurfaceComponent({ match, perspectiveSeat = 0, hiddenCardIds, hideAllPlayedCards, centerOverlay }: TableSurfaceProps) {
  ...
  // Retard reactiu de l'apilat: quan una baza es tanca (4 cartes + guanyador),
  // no ombregem les cartes immediatament. En lloc d'això, esperem 1 s. Si el
  // jugador següent tira la seua carta abans, el mecanisme natural
  // (displayTrick amb 1 carta ⇒ slidingCardIdByPlayer) ja dispara l'ombrejat
  // simultàniament a la seua acció. Si ningú tira en 1 s, el timer flipa
  // `shadeReady=true` i les cartes s'apilen automàticament perquè el pròxim
  // jugador (humà o bot) trobe la mesa "neta".
  const closedTrickKey = lastTrickClosed && lastTrick
    ? `${r.tricks.length}:${lastTrick.cards[0]?.card.id ?? ""}`
    : null;
  const [shadeReady, setShadeReady] = useState(false);
  const prevClosedKeyRef = useRef<string | null>(null);
  useEffect(() => {
    if (!closedTrickKey) {
      prevClosedKeyRef.current = null;
      if (shadeReady) setShadeReady(false);
      return;
    }
    if (prevClosedKeyRef.current === closedTrickKey) return;
    prevClosedKeyRef.current = closedTrickKey;
    setShadeReady(false);
    const t = window.setTimeout(() => setShadeReady(true), 1000);
    return () => window.clearTimeout(t);
  }, [closedTrickKey, shadeReady]);

  const shouldShadeClosedLastTrick =
    lastTrickClosed && !roundHasEnded && shadeReady;
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isWinner ? "z-[20]" : isLatestPlayer ? "z-[10]" : "z-0",
              )}
              style={{
                left: pos.x,
                top: pos.y,
                ...
                {prev.map((c, i) => {
                  ...
                  return (
                    <div
                      ...
                      className="absolute top-1/2 -translate-y-1/2 transition-[margin-top,left] duration-[450ms] ease-out"
                      style={{
                        left: `${finalLeftPx}px`,
                ...
                {currentTC && (() => {
                  ...
                  return (
                    <div
                      ...
                      style={{ zIndex: isWinner ? 20 : 10, ...(playStyle ?? {}) }}
                    >
                      <PlayingCard
                      ...
                      />
                      {isWinner && (
                        <div className="absolute -inset-1 rounded-card border-2 border-primary animate-pulse-gold pointer-events-none z-[30]" />
                      )}
                    </div>
                  );
                })()}
