...
export function EndGameOverlay({
  ...
  return (
    <Dialog open={open}>
      <DialogContent
        className="max-w-md w-[calc(100%-3rem)] rounded-2xl border-2 border-gold gold-glow gap-2 [&>button]:hidden pt-[44px] pb-[44px]"
        style={{ zIndex: TRUC_Z_INDEX.endGameOverlay }}
        onPointerDownOutside={(e) => e.preventDefault()}
        onEscapeKeyDown={(e) => e.preventDefault()}
      >
        <DialogHeader>
          <DialogTitle className="font-title font-black italic text-gold text-2xl text-center mb-[30px]">
            {t("match.end.title")}
          </DialogTitle>
        </DialogHeader>

        <div className="flex flex-col items-center gap-3 text-center -mt-[70px]">
          ...
          <p className="text-xs text-muted-foreground mt-1 mb-[30px]">
            {jocFora
        ...
        <div className="flex flex-col gap-2 mt-2">
          <Button
            onClick={onNewGame}
            className="justify-center h-auto min-h-[44px] py-2 text-[15px] bg-primary text-primary-foreground hover:bg-primary/90 font-display font-bold gold-glow uppercase tracking-wide"
          >
          ...
          <Button
            onClick={onAbandon}
            variant="destructive"
            className="justify-center h-auto min-h-[44px] py-2 text-[15px] font-display font-bold uppercase tracking-wide"
          >
