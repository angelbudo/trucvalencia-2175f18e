...
export interface TrucBoardProps {
  ...
  /** Cant acceptat implícitament per una escalada del rival (retruc/quatre/joc-fora).
   *  Quan és cert, la V verda apareix a l'instant sense esperar cap "Vull". */
  implicitAcceptedByPlayer?: Record<PlayerId, boolean>;
  ...
  /** Envit acceptat implícitament per una escalada del rival/company
   *  (renvit / falta-envit). Quan és cert, la V verda del cantador previ
   *  es pinta immediatament sense esperar cap "Vull". */
  implicitAcceptedEnvitByPlayer?: Record<PlayerId, boolean>;
  ...
  /** Noms específics per seient a mostrar a la pantalla final de la partida.
   *  Permet forçar noms diferents dels que s'usen durant el joc (per
   *  exemple, "Company" en lloc de "Compañero" o "Bot Izq"/"Bot Der" en
   *  mode local contra bots). Si no es proporciona, s'usen els seatNames. */
  endgameSeatNames?: Record<PlayerId, string>;
...
export function TrucBoard(props: TrucBoardProps) {
  ...
  const implicitAcceptedByPlayer = props.implicitAcceptedByPlayer ?? { 0: false, 1: false, 2: false, 3: false };
  ...
  const implicitAcceptedEnvitByPlayer = props.implicitAcceptedEnvitByPlayer ?? { 0: false, 1: false, 2: false, 3: false };
  ...
  // Resolució visual "encunyada" (locked-in) per jugador. Una vegada
  // s'ha vist la V verda o la X roja d'un cartell, la conservem fins al
  // final de la mà o fins que el mateix jugador puge la seua aposta —
  // encara que després arribe una nova pregunta (retruc, renvit...) que
  // reseteja el registre de respostes visibles. Així el cartell del
  // primer cantador manté la seua marca mentre el nou cantador queda
  // en espera de la resposta del rival.
  const [lockedResolution, setLockedResolution] = useState<Record<PlayerId, "V" | "X" | null>>({
    0: null, 1: null, 2: null, 3: null,
  });
  // Anàleg per als carteles d'envit: encunyem "volgut" / "no-volgut" una
  // vegada s'ha vist la resposta del rival, per no perdre la marca si més
  // tard arriba una nova pregunta que reseteje les respostes visibles.
  const [lockedEnvitOutcome, setLockedEnvitOutcome] = useState<Record<PlayerId, "volgut" | "no-volgut" | null>>({
    0: null, 1: null, 2: null, 3: null,
  });
  ...
  useEffect(() => {
    ...
    if (respondersRoundRef.current !== match.history.length) {
      ...
      setLockedResolution({ 0: null, 1: null, 2: null, 3: null });
      setLockedEnvitOutcome({ 0: null, 1: null, 2: null, 3: null });
    }
  }, [match.history.length, r.phase]);
  // Cada vegada que arriba una nova "pregunta" (truc/retruc/quatre/joc-fora
  // o envit/renvit/falta-envit) hem de resetejar les respostes visibles
  // acumulades: el nou cantador ha de mostrar la V/X només quan el rival
  // responga a la seua pregunta, no aprofitant respostes anteriors. La
  // marca ja "encunyada" per als cantadors anteriors es conserva en
  // `lockedResolution`; al nou cantador li esborrem qualsevol lock previ
  // (el seu cartell s'acaba d'actualitzar i torna a estar en espera).
  const lastQuestionShoutIdxRef = useRef<number>(-1);
  useEffect(() => {
    let latestIdx = -1;
    for (let i = r.log.length - 1; i >= 0; i--) {
      const ev = r.log[i];
      if (ev.type === "shout" && ev.what !== "vull" && ev.what !== "no-vull" && ev.what !== "passe" && ev.what !== "so-meues") {
        latestIdx = i;
        break;
      }
    }
    if (latestIdx > lastQuestionShoutIdxRef.current) {
      const ev = r.log[latestIdx];
      lastQuestionShoutIdxRef.current = latestIdx;
      if (ev.type === "shout") {
        setRespondersSeen({ 0: null, 1: null, 2: null, 3: null });
        setLockedResolution((prev) => {
          if (prev[ev.player] === null) return prev;
          return { ...prev, [ev.player]: null };
        });
        setLockedEnvitOutcome((prev) => {
          if (prev[ev.player] === null) return prev;
          return { ...prev, [ev.player]: null };
        });
      }
    } else if (latestIdx < lastQuestionShoutIdxRef.current) {
      // Nova mà: el log s'ha reiniciat.
      lastQuestionShoutIdxRef.current = latestIdx;
    }
  }, [r.log]);
  useEffect(() => {
  ...
  // Regla visual invariant: l'interrogant "?" sobre l'avatar d'un
  // jugador no pot desaparèixer abans que el bocadillo central del
  // seu cant siga visible al tauler. Els temporitzadors interns
  // (audio-lead, gap entre cartells, cua serial de `useShoutFlashes`)
  // introdueixen un desfasament entre el moment en què l'engine
  // registra el cant al `log` i el moment en què `shoutFlashes` el
  // mostra. Per a garantir un enllaç exacte en el mateix render,
  // marquem com a "held" qualsevol jugador amb un cant al log que
  // encara no haja aparegut com a cartell central. L'effect de neteja
  // (líneas 555-605) esborra el "held" en el mateix render en què el
  // cartell es fa visible, així "?" i bocadillo commutant al unison.
  //
  // Només marquem esdeveniments NOUS del log (els que encara no hem
  // processat mai): així evitem re-activar "?" un cop el bocadillo
  // ja s'ha ocultat de forma natural. La neteja al effect posterior
  // sincronitza la desaparició de "?" amb l'aparició del cartell.
  const shoutSeenIdxRef = useRef<number>(-1);
  const shoutHeldRoundRef = useRef<number>(match.history.length);
  useIsomorphicLayoutEffect(() => {
    if (shoutHeldRoundRef.current !== match.history.length) {
      shoutHeldRoundRef.current = match.history.length;
      shoutSeenIdxRef.current = -1;
    }
    const pending: PlayerId[] = [];
    for (let i = shoutSeenIdxRef.current + 1; i < r.log.length; i++) {
      const ev = r.log[i];
      if (ev.type === "shout") pending.push(ev.player);
    }
    shoutSeenIdxRef.current = r.log.length - 1;
  ...
  // Fallback per a rehidratació (reanudar partida guardada): si el log ja
  // conté les respostes "vull"/"no-vull" posteriors a l'última pregunta,
  // considerem la resposta completa encara que els flashes visuals no
  // s'hagen reproduït (mai apareixeran per a events antics).
  const respondersFromLog = useMemo(() => {
    const out: Record<PlayerId, "vull" | "no-vull" | null> = {
      0: null, 1: null, 2: null, 3: null,
    };
    let lastQ = -1;
    for (let i = r.log.length - 1; i >= 0; i--) {
    ...
    for (let i = lastQ + 1; i < r.log.length; i++) {
      const ev = r.log[i];
      if (ev.type === "shout" && (ev.what === "vull" || ev.what === "no-vull")) {
        out[ev.player] = ev.what;
      }
    }
    return out;
  }, [r.log]);
  const opponentResponseComplete = (
    ...
    const responseOf = (p: PlayerId) => respondersSeen[p] ?? respondersFromLog[p];
    if (outcome === "volgut") {
      return opponents.some((p) => responseOf(p) === "vull");
    }
    return opponents.every((p) => responseOf(p) === "no-vull");
  };
  ...
  // "pending" (no es pinta marca i el cartell continua pulsant). Una
  // vegada s'ha vist la resposta, encunyem el resultat en
  // `lockedEnvitOutcome` per preservar-lo si més tard el rival puja
  // l'aposta amb un renvit/falta-envit.
  const effectiveEnvitOutcome = (
    p: PlayerId,
  ): "pending" | "volgut" | "no-volgut" | null => {
    const o = envitOutcomeByPlayer?.[p];
    if (!o) return null;
    if (lockedEnvitOutcome[p] && o.outcome !== "pending") return lockedEnvitOutcome[p];
    if (o.outcome === "pending") return "pending";
    // Acceptació implícita: pinta la V a l'instant, sense esperar cap "Vull".
    if (implicitAcceptedEnvitByPlayer[p] && o.outcome === "volgut") return "volgut";
    if (!opponentResponseComplete(p, o.outcome)) return "pending";
    return o.outcome;
  };
  // V verda del truc: només es pinta una vegada s'ha vist el cartell
  // central "Vull" del rival. Si ja s'havia mostrat prèviament (i s'ha
  // encunyat a `lockedResolution`), es conserva encara que arribe una
  // nova pregunta que reseteje les respostes visibles.
  const effectiveAcceptedShout = (p: PlayerId): boolean => {
    if (!acceptedShoutByPlayer[p]) return false;
    if (lockedResolution[p] === "V") return true;
    // Acceptació implícita per escalada del rival: la V es pinta a l'instant
    // (no hi haurà cap "Vull" central del rival que ho desbloqueje).
    if (implicitAcceptedByPlayer[p]) return true;
  ...
  const effectiveRejectedShout = (p: PlayerId): boolean => {
    ...
    if (lockedResolution[p] === "X") return true;
    return opponentResponseComplete(p, "no-volgut");
  };

  // Encunyem la V/X en `lockedResolution` en el mateix instant en què
  // esdevé visible. Així sobreviu a un futur reset de `respondersSeen`
  // provocat per una nova pregunta del rival o del company.
  useEffect(() => {
    setLockedResolution((prev) => {
      let next = prev;
      let changed = false;
      for (const p of [0, 1, 2, 3] as PlayerId[]) {
        if (prev[p] === null) {
          if (acceptedShoutByPlayer[p] && (implicitAcceptedByPlayer[p] || opponentResponseComplete(p, "volgut"))) {
            if (!changed) { next = { ...prev }; changed = true; }
            next[p] = "V";
          } else if (rejectedShoutByPlayer[p] && opponentResponseComplete(p, "no-volgut")) {
            if (!changed) { next = { ...prev }; changed = true; }
            next[p] = "X";
          }
        }
    ...
    setLockedEnvitOutcome((prev) => {
      ...
      let changed = false;
      for (const p of [0, 1, 2, 3] as PlayerId[]) {
        if (prev[p] !== null) continue;
        const o = envitOutcomeByPlayer?.[p];
        if (!o || o.outcome === "pending") continue;
        const implicit = implicitAcceptedEnvitByPlayer[p] && o.outcome === "volgut";
        if (implicit || opponentResponseComplete(p, o.outcome)) {
          if (!changed) { next = { ...prev }; changed = true; }
          next[p] = o.outcome;
        }
      }
  ...
  }, [acceptedShoutByPlayer, rejectedShoutByPlayer, respondersSeen, respondersFromLog, envitOutcomeByPlayer, implicitAcceptedByPlayer, implicitAcceptedEnvitByPlayer]);
  ...
  // mà i la recollida de cartes. Es mostra durant ~4,5 s perquè la bombolla
  // guanyadora estiga 3 s a escala 1.
  ...
  // Refs per a l'ancoratge dinàmic de les mans laterals al slot de la
  // carta jugada del jugador corresponent. Fem servir getBoundingClientRect
  // en un useLayoutEffect + resize observer per garantir precisió a
  // qualsevol resolució/orientació (vegeu blocs JSX més avall).
  ...
  useIsomorphicLayoutEffect(() => {
    const align = () => {
      ...
      const pairs: Array<[HTMLDivElement | null, HTMLDivElement | null]> = [
        [leftSlotRef.current, leftHandRef.current],
        [rightSlotRef.current, rightHandRef.current],
      ];
      for (const [slot, hand] of pairs) {
        if (!slot || !hand) continue;
        const s = slot.getBoundingClientRect();
        const centerY = s.top + s.height / 2 - boardRect.top;
        hand.style.top = `${centerY}px`;
      }
    };
    align();
    const ro = typeof ResizeObserver !== "undefined" ? new ResizeObserver(align) : null;
    if (ro && boardAreaRef.current) ro.observe(boardAreaRef.current);
  ...
  // Mentre quede algun jugador "held" (ha emès un cant que encara no ha
  // aparegut com a bocadillo central), bloqueja també l'interrogant de
  // "torn actiu" del pròxim jugador. Així el "?" del que ara ha de tirar
  // no apareix abans que el bocadillo (Vull, No vull, envit, etc.) siga
  // visible: mateixa regla de sincronització que fem servir per a la
  // desaparició de l'interrogant.
  const suppressTurnQuestion = ([0, 1, 2, 3] as PlayerId[]).some(
    (p) => heldPending[p],
  );
  ...
  // Regla universal: si la 1a baza va quedar empardada, la carta que li
  // queda a la mà a cada jugador que ja ha jugat en la 2a baza s'oculta
  // visualment (per a bots, company i humà per igual). Purament estètic:
  // l'estat intern de `handsView`/`r.hands` conserva la carta i el càlcul
  // d'envit la segueix comptant íntegrament.
  const firstParda = r.tricks[0]?.parda === true;
  const secondTrickStarted = r.tricks.length >= 2;
  const hideRemainingForSeat = (seat: PlayerId): boolean =>
    firstParda &&
    secondTrickStarted &&
    !!r.tricks[1]?.cards.some((tc) => tc.player === seat);
  ...
  return (
    <main ref={boardRootRef} data-paused={paused ? "true" : undefined} className="min-h-screen flex flex-col relative pt-5">
        ...
        <div className="fixed inset-0 bg-background/70 backdrop-blur-sm flex flex-col items-center justify-center gap-4 pointer-events-auto" style={{ zIndex: TRUC_Z_INDEX.pauseOverlay }}>
          ...
          {onPauseToggle && (
            <Button
              ...
              variant="outline"
              className="border-primary/60 text-primary hover:bg-primary/10 uppercase font-semibold tracking-wide"
            >
      ...
      <div ref={boardAreaRef} className="flex-1 shrink-0 relative mt-[95px] mb-1 mx-2 min-h-[480px]">
        {/* Ancoratges invisibles que reprodueixen la geometria dels slots de
            carta jugada dins del felt de TableSurface (absolute inset-5,
            posicions 28%/50% i 72%/50%). Els fem servir com a referència
            per alinear verticalment les mans laterals via useRef +
            getBoundingClientRect (vegeu useLayoutEffect a dalt). */}
        ...
        <div className="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 z-[30]" style={{ top: "25px" }} data-deck-anchor={PARTNER}>
          <HiddenHand
            count={Math.max(0, collectOverlayReady ? 0 : ((dealing || passing) ? Math.min(revealedCount[PARTNER], 3) : (hideRemainingForSeat(PARTNER) ? 0 : handsView[PARTNER].length)))}
              ...
              <div className="absolute bottom-full mb-[3px] left-1/2 -translate-x-1/2">
                <div className="relative inline-block">
                  ...
                  {(() => { const _o = effectiveEnvitOutcome(PARTNER); return _o ? (
                    <EnvitOutcomeMark
                      outcome={_o}
                      className="absolute left-full top-1/2 -translate-y-1/2 -ml-[2px]"
                    />
                  ) : null; })()}
                </div>
              </div>
        ...
        <div ref={leftHandRef} className="absolute left-0 z-[30] flex items-center justify-center" style={{ top: "50%", transform: "translateY(calc(-50% - 38px))" }} data-deck-anchor={LEFT}>
          <HiddenHand
            count={Math.max(0, collectOverlayReady ? 0 : ((dealing || passing) ? Math.min(revealedCount[LEFT], 3) : (hideRemainingForSeat(LEFT) ? 0 : handsView[LEFT].length)))}
            direction="vertical"
        ...
        <div className="absolute left-1 top-[43.5%] z-50" style={{ marginTop: "60px" }}>
          <div className="relative z-50">
            <PlayerSeat player={LEFT} match={match} position="left" name={nameLeft} isPendingResponder={isPendingResponder(LEFT)} suppressTurnQuestion={suppressTurnQuestion} presence={presenceFor(LEFT)} presenceLastSeen={presenceLastSeenFor(LEFT)} avatarUrl={avatarFor(LEFT)} secondsLeft={secondsLeftFor(LEFT)} profileUserId={profileUserIdFor(LEFT)} profileDeviceId={profileDeviceIdFor(LEFT)}  />
            {(() => {
        ...
        <div className="absolute right-1 top-[43.5%] z-40" style={{ transform: "translateY(-100%)", marginTop: "-70px" }}>
          <div className="relative z-50">
            <PlayerSeat player={RIGHT} match={match} position="right" name={nameRight} isPendingResponder={isPendingResponder(RIGHT)} suppressTurnQuestion={suppressTurnQuestion} presence={presenceFor(RIGHT)} presenceLastSeen={presenceLastSeenFor(RIGHT)} avatarUrl={avatarFor(RIGHT)} secondsLeft={secondsLeftFor(RIGHT)} profileUserId={profileUserIdFor(RIGHT)} profileDeviceId={profileDeviceIdFor(RIGHT)}  />
            {(() => {
              ...
              <div className="absolute bottom-full mb-[3px] left-1/2 -translate-x-1/2">
                <div className="relative inline-block">
                  ...
                  {(() => { const _o = effectiveEnvitOutcome(RIGHT); return _o ? (
                    <EnvitOutcomeMark
                      outcome={_o}
                      className="absolute right-full top-1/2 -translate-y-1/2 -mr-[2px]"
                    />
                  ) : null; })()}
                </div>
              </div>
        ...
        <div ref={rightHandRef} className="absolute right-0 z-[30] flex items-center justify-center" style={{ top: "50%", transform: "translateY(calc(-50% - 38px))" }} data-deck-anchor={RIGHT}>
          <HiddenHand
            count={Math.max(0, collectOverlayReady ? 0 : ((dealing || passing) ? Math.min(revealedCount[RIGHT], 3) : (hideRemainingForSeat(RIGHT) ? 0 : handsView[RIGHT].length)))}
            direction="vertical"
          ...
          >
            <div className="relative">
              ...
              {envitShoutByPlayer?.[HUMAN] && (
                <div className="absolute bottom-full mb-[3px] left-1/2 -translate-x-1/2" style={{ zIndex: TRUC_Z_INDEX.shout }}>
                  <div className="relative inline-block">
                    <ShoutBadge
          ...
          <div className="flex-1 relative flex flex-col items-center justify-end min-w-0 -ml-[10px] h-[112px]">
            ...
            {myHand.length === 0 ? (
              ...
              myHand.map((c, i) => {
                ...
                // Regla: si la 1a baza ha quedat empardada, la 2a baza és
                // decisiva. Quan l'humà juga la seua carta en la 2a baza,
                // la carta que li queda a la mà s'oculta visualment (com
                // si quedara amagada davall la carta jugada). L'ocultació
                // és PURAMENT ESTÈTICA: l'estat intern de `myHand` conserva
                // la carta i el càlcul d'envit (`myEnvit`, `bestEnvit`) la
                // segueix comptant íntegrament.
                {
                  if (hideRemainingForSeat(HUMAN)) {
                    return <div key={c.id} className="w-0 h-[112px]" aria-hidden="true" />;
                  }
                }
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "relative z-[50] flex flex-col items-center gap-1 transition-all duration-300 ease-out",
      ...
      {gameEnded && (() => {
        ...
        const playerNamesBySeat: Record<PlayerId, string> = {
          [HUMAN]: endgameSeatNames?.[HUMAN] ?? seatNames?.bottom ?? `Seient ${HUMAN + 1}`,
          [RIGHT]: endgameSeatNames?.[RIGHT] ?? seatNames?.right ?? `Seient ${RIGHT + 1}`,
          [PARTNER]: endgameSeatNames?.[PARTNER] ?? seatNames?.top ?? `Seient ${PARTNER + 1}`,
          [LEFT]: endgameSeatNames?.[LEFT] ?? seatNames?.left ?? `Seient ${LEFT + 1}`,
      ...
      <AlertDialog open={confirmAbandon} onOpenChange={setConfirmAbandon}>
        <AlertDialogContent
          className="z-[9100] max-w-md w-[calc(100%-3rem)] rounded-2xl border-primary/30"
        ...
        >
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="uppercase font-semibold tracking-wide">{t("common.cancel")}</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
                setConfirmAbandon(false);
                onAbandon();
              }}
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90 uppercase font-semibold tracking-wide"
            >
              {t("match.abandon_btn")}
            </AlertDialogAction>
          </AlertDialogFooter>
