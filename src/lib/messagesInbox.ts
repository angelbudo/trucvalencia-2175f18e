...
export const ADMIN_NOTICE_CATEGORY = "Avisos del Administrador";

type InboxSnapshot = {
  userId: string;
  messages: InboxMessage[];
  adminNotices: InboxMessage[];
...
async function fetchInbox(userId: string): Promise<InboxSnapshot> {
  ...
  const enriched = rawMessages.map((x) => {
  ...
  const adminNotices = enriched.filter((x) => x.category === ADMIN_NOTICE_CATEGORY);
  const messages = enriched.filter((x) => x.category !== ADMIN_NOTICE_CATEGORY);
...
export function useInbox(userId: string | null | undefined) {
  ...
  return {
    ...
    adminNotices: snap?.adminNotices ?? [],
