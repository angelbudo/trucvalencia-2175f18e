  ...
  useEffect(() => {
    ...
    for (let i = start; i < log.length; i++) {
      ...
      queueTailRef.current = queueTailRef.current.then(async () => {
        ...
        // Sincronització estricta: el cartell central i la locució
        // s'activen exactament al mateix instant que la nova actualització
        // de gameState (el mateix disparador que fa aparèixer el cartell
        // en l'indicador del jugador). No introduïm cap "audio lead":
        // qualsevol retard fa que la UI se senta lenta i desincronitzada.
        const speakNow = () =>
