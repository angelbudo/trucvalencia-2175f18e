...
export function useTrucMatch(options: UseTrucMatchOptions = {}) {
  ...
  const lastPhraseByPlayerRef = useRef<Map<PlayerId, string>>(new Map());

  const sayRef = useRef<UseTrucMatchOptions["say"]>(undefined);
  sayRef.current = (player, phraseId, durationMs, vars) => {
    // Deduplicació: si el bot està a punt d'emetre exactament la mateixa
    // frase que la seua última intervenció, es descarta per no ensuciar
    // el xat amb missatges idèntics consecutius (p. ex. quan es creuen
    // esdeveniments de xat entre bots).
    const key = `${String(phraseId).trim().toLowerCase()}|${JSON.stringify(vars ?? null)}`;
    if (player !== 0 && lastPhraseByPlayerRef.current.get(player) === key) {
      return;
    }
    lastPhraseByPlayerRef.current.set(player, key);
  ...
  const implicitAcceptedByPlayer = display.implicitAcceptedByPlayer;
  ...
  const implicitAcceptedEnvitByPlayer = display.implicitAcceptedEnvitByPlayer;
  ...
  const enqueueLocalShoutFlash = useCallback((player: PlayerId, what: ShoutKind, labelOverride?: string) => {
    ...
    // Sincronització estricta: cartell central i indicador de jugador
    // s'activen amb el mateix disparador (mateixa actualització de match).
    // Sense "audio lead": la veu i el cartell arrenquen alhora.
    const estimatedStartAt = Math.max(Date.now(), localFlashBusyUntilRef.current);
    localFlashBusyUntilRef.current = estimatedStartAt + SHOUT_FLASH_HOLD_MS + SHOUT_FLASH_GAP_MS;
    localFlashTailRef.current = localFlashTailRef.current.then(async () => {
      ...
      if (hadVisible) {
        localFlashVisibleRef.current = [];
        setLocalFlashQueue([]);
        // Només introduïm el gap quan hi ha dos cartells consecutius que
        // s'han de separar; si aquest és aïllat, apareix instantàniament.
      ...
      // Mostra el cartell i dispara la veu al mateix instant.
      ...
      localFlashVisibleRef.current = [];
      setLocalFlashQueue([]);
      // Gap opcional només si arriba un segon cartell tot seguit: el `hadVisible`
      // de la següent iteració l'aplicarà. Aquí no bloquegem la cua més enllà
      // del hold del cartell.
    });
  }, []);
  ...
  const dispatch = useCallback((player: PlayerId, action: Action) => {
    ...
    // Seña automàtica "Fica algo fort": si un bot té 3 treses en la
    // mà inicial i tira el seu PRIMER tres en la Baza 1, avisa al
    // company perquè col·labori amb una carta forta.
    if (action.type === "play-card" && player !== HUMAN) {
      const prev = matchRef.current;
      if (prev) {
        const pr = prev.round;
        const inBaza1 = pr.tricks.length === 1;
        const hand = pr.hands[player] ?? [];
        const card = hand.find((c) => c.id === action.cardId);
        const noneOfMinePlayed = !pr.tricks.some((t) =>
          t.cards.some((tc) => tc.player === player),
        );
        if (
          inBaza1 &&
          hand.length === 3 &&
          hand.every((c) => c.rank === 3) &&
          card &&
          card.rank === 3 &&
          noneOfMinePlayed
        ) {
          sayRef.current?.(player, "pon-fort");
        }
      }
    }
    ...
    if (action.type === "shout") {
      const what = action.what;
      // Regla d'obligació encadenada: només s'activa si el jugador canta
      // un ENVIT INICIAL (no renvit/falta-envit) dins del seu torn
      // principal a la PRIMERA BAZA — abans d'haver tirat cap carta en
      // eixa baza. Un re-envit posterior és una acció diferent i NO ha
      // de disparar l'obligació de trucar.
      if (what === "envit") {
        const pr = matchRef.current?.round;
        const prMano = pr?.mano;
        const firstTrick = pr?.tricks[0];
        const inFirstTrick = !!pr && pr.tricks.length === 1;
        const playerHasNotPlayedYet =
          !!firstTrick && !firstTrick.cards.some((tc) => tc.player === player);
        if (
          prMano !== undefined &&
          inFirstTrick &&
          playerHasNotPlayedYet &&
          isFirstOfPair(player, prMano)
        ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (pendingSecondWaitRef.current?.waitKey !== mandatoryWaitKey) {
        ...
        const proactiveMandatoryEnvitKey = `pro-envit-mandatory-${match.history.length}-${r.mano}-${botPlayer}-${mandatoryFirstTrick.cards.length}`;
        if (
          sayRef.current &&
          mandatoryPartner !== HUMAN &&
          mandatoryBotCanEnvitNow &&
          shouldProactivelyOrderEnvit(playerTotalEnvit(r, mandatoryPartner)) &&
          !proactiveEnvitInstructionRef.current.has(proactiveMandatoryEnvitKey)
        ) {
          proactiveEnvitInstructionRef.current.add(proactiveMandatoryEnvitKey);
          partnerBotTimer = window.setTimeout(() => {
            const current = pendingSecondWaitRef.current;
            if (current?.waitKey !== mandatoryWaitKey) return;
            sayRef.current?.(mandatoryPartner, "envida");
    ...
    // Pausa "dramàtica" de reacció unificada: sempre que el company del
    // bot haja emès un chat (frase, pregunta, senyal, consell) en els
    // últims 1500 ms, el bot espera EXACTAMENT 1500 ms des d'eixe
    // missatge abans d'executar la seua acció (tirar carta, cantar,
    // etc.). Regla única per a totes les interaccions entre companys,
    // eliminant les antigues barreges de 500 ms / 1000 ms.
    {
      const PARTNER_CHAT_REACTION_MS = 1500;
      const partnerChatAt = lastChatAtByPlayerRef.current.get(partnerOf(botPlayer)) ?? 0;
      const sincePartnerChat = Date.now() - partnerChatAt;
      if (partnerChatAt > 0 && sincePartnerChat < PARTNER_CHAT_REACTION_MS) {
        delay = Math.max(delay, PARTNER_CHAT_REACTION_MS - sincePartnerChat);
      }
    }
    ...
    if (
      ...
      // Previsualitza què faria el bot SENSE consultar. Aplica la
      // consulta tàctica al company sempre que la seua decisió natural
      // NO siga un rebuig clar (no-vull). Així cobrim tots els casos en
      // què el bot podria acabar acceptant (vull) o pujant, forçant-lo
      // a preguntar abans al company per si prefereix pujar.
      ...
      const previewIsReject =
        previewAction?.type === "shout" && previewAction.what === "no-vull";
      const wouldAccept =
        !previewIsReject &&
        canRaiseOrAccept &&
          ...
          initiativeChatTimers.start([
            {
              ...
              at: PARTNER_BOT_INSTRUCTION_DELAY_MS,
              fn: () => {
  ...
  useEffect(() => {
    if (isEngineLocked()) return;
    const r = match.round;
    // Regla "envit i truca": si el motor marca l'humà com a obligat a
    // cantar truc (per haver iniciat envit o falta-envit com a primer
    // de la seua pareja en la 1a baza), disparem el truc automàticament
    // sense requerir cap clic. Cobreix tant envit inicial com falta-envit
    // inicial. Es respecta a més la neteja del ref per si algun altre
    // camí encara l'esperava.
    if (r.chainedTrucPending !== HUMAN) return;
    if (r.envitState.kind === "pending") return;
